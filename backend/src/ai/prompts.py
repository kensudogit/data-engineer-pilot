"""Shared, execution-mode-agnostic prompt builders for the "AI insight"
narrative summary feature.

Used by both src/snowflake/cortex/insight.py (SNOWFLAKE.CORTEX.COMPLETE,
execution_mode="snowflake") and src/ai/openai_client.py (OpenAI, an optional
enhancement to the demo path) — the prompt text itself doesn't reference
either backend, so one set of builders serves both.

Every build_prompt_* function reads only already-computed aggregate metrics
(AUC, silhouette score, MAE/RMSE, cluster sizes, counts) — never raw
customer rows or PII — since the resulting text is meant to summarize a
model's results, not analyze individual customers.
"""
